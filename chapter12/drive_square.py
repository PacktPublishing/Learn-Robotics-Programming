...
def drive_distances(bot, left_distance, right_distance, speed=80):
    ...
    while abs(primary_encoder.pulse_count) < abs(primary_distance) or abs(secondary_encoder.pulse_count) < abs(secondary_distance):
        ...
        # Where do we want the secondary to be?
        ...
        if primary_encoder.pulse_count >= primary_distance:
            print("primary stop")
            set_primary(0)
            secondary_speed = 0
...
bot = Robot()
print "Bot wheel diameter", bot.wheel_diameter_mm, "Bot wheel distance:", bot.wheel_distance_mm
